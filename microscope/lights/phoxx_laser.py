...
class PhoxXLaser(microscope.abc.SerialDeviceMixin, microscope.abc.LightSource):
    ...
    def _initialize_device(self):
        ...
        print(f"firmware: {firmware} (type: {type(firmware)})")
    ...
    def _do_trigger(self):
        # Not used — raise because we only support hardware triggering
        raise NotImplementedError("Software trigger is not supported for PhoxXLaser.")

    @microscope.abc.SerialDeviceMixin.lock_comms
    def set_trigger(self, mode: TriggerMode, trigger_type: TriggerType):
        if mode != TriggerMode.HARDWARE:
            raise ValueError("PhoxXLaser only supports hardware triggering.")
